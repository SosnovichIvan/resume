...
export function PersonalProjectCard({ project }: PersonalProjectCardProps) {
	return <Card hover className="flex h-full flex-col p-6">
		<div className="mb-4 flex items-start gap-3"><Image src={project.logo} alt={`Логотип ${project.name}`} width={48} height={48} className="h-12 w-12 shrink-0 rounded-xl" /><div className="min-w-0"><Link href={`/my-projects/${project.slug}`} className="text-xl font-semibold transition-colors hover:text-brand-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 dark:hover:text-brand-300">{project.name}</Link><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{project.role}</p></div></div>
		<p className="mb-4 flex-1 text-sm leading-relaxed text-slate-600 dark:text-slate-300">{project.description}</p>
		<p className="mb-4 flex items-start gap-2 text-xs font-medium leading-relaxed text-slate-500 dark:text-slate-400"><Icon name="sparkles" className="mt-0.5 h-4 w-4 shrink-0 text-brand-600 dark:text-brand-300" />{project.creationProcess}</p>
		<div className="mb-5 rounded-xl border border-brand-200 bg-white px-4 py-3 text-sm font-bold leading-relaxed text-slate-950 shadow-sm dark:border-brand-800 dark:bg-slate-950 dark:text-white"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-brand-600 dark:text-brand-300">Главный результат</span>{project.featuredResult}</div>
